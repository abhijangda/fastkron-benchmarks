...
int size_internal)
{
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'y', 't2', ['e', 'c1', 'c2']], [16, 'STR_SD2_V2_H7', 'x', 'v2', ['a', 'b', 'e', 'd']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			sm_a[threadIdx.x][threadIdx.y + ll * 16] = dev_t2[(blk_idx_c1 * SIZE_SLICE_1_C1 + idx_c1 + (blk_idx_c2 * SIZE_SLICE_1_C2 + ll) * size_c1) * size_e + (threadIdx.x + l)];
		}
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			sm_b[threadIdx.y][threadIdx.x + ll * 16] = dev_v2[blk_idx_a * SIZE_SLICE_1_A + idx_a + (blk_idx_b * SIZE_SLICE_1_B + ll + ((blk_idx_d * SIZE_SLICE_1_D + 0) * size_e) * size_b) * size_a + (threadIdx.y + l) * stride_int_v2];
		}
		__syncthreads();
		//---------------------------------------------------------------------------------------------------
		...
		{
			temp_bv[0] = sm_a[ll][idx_c1 + 0];
			temp_bv[1] = sm_a[ll][idx_c1 + 16];
			temp_bv[2] = sm_a[ll][idx_c1 + 32];
			temp_bv[3] = sm_a[ll][idx_c1 + 48];
			temp_bv[4] = sm_a[ll][idx_c1 + 64];
			temp_bv[5] = sm_a[ll][idx_c1 + 80];
...
void sd_t_d2_fusion(int size_a, int size_b, int size_c1, int size_c2, int size_d, int size_e, double* t3, double* host_t2, double* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	cudaMalloc((void**) &dev_t2, sizeof(double) * size_c2 * size_c1 * size_e);
	cudaMalloc((void**) &dev_v2, sizeof(double) * size_d * size_e * size_b * size_a);

	// cudaMemcpy()
	cudaMemcpy(dev_t3, t3, sizeof(double) * size_a * size_b * size_c1 * size_c2 * size_d, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_c2 * size_c1 * size_e, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_d * size_e * size_b * size_a, cudaMemcpyHostToDevice);
