...
#define SIZE_TB_1_X 	SIZE_SLICE_1_A * SIZE_SLICE_1_C * SIZE_SLICE_1_B
...
double* dev_t2, 
double* dev_v2, 
int size_a, int size_b, int size_c, int size_d1, int size_d2, int size_e, int size_f, 
int numBlk_a, int numBlk_b, int numBlk_c, int numBlk_d1, int numBlk_d2, int numBlk_e, 
...
int size_internal)
{
	...
	int idx_b = threadIdx.x / (SIZE_SLICE_1_C * SIZE_SLICE_1_A);
	...
	int tmp_blkIdx;
	int blk_idx_e = blockIdx.x / (numBlk_d2 * numBlk_d1 * numBlk_c * numBlk_b * numBlk_a);
	tmp_blkIdx = blockIdx.x % (numBlk_d2 * numBlk_d1 * numBlk_c * numBlk_b * numBlk_a);

	int blk_idx_d2 = tmp_blkIdx / (numBlk_d1 * numBlk_c * numBlk_b * numBlk_a);
	tmp_blkIdx = tmp_blkIdx % (numBlk_d1 * numBlk_c * numBlk_b * numBlk_a);

	int blk_idx_d1 = tmp_blkIdx / (numBlk_c * numBlk_b * numBlk_a);
	...
	int t3_base_thread = blk_idx_a * SIZE_SLICE_1_A + idx_a + (blk_idx_b * SIZE_SLICE_1_B + idx_b + (blk_idx_c * SIZE_SLICE_1_C + idx_c + (blk_idx_d1 * SIZE_SLICE_1_D1 + idx_d1 + (blk_idx_d2 * SIZE_SLICE_1_D2 + (blk_idx_e * SIZE_SLICE_1_E) * size_d2) * size_d1) * size_c) * size_b) * size_a;
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['e', 'c', 'b', 'f', 'a']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['f', 'd1', 'd2']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 4; ll++)
		{
			...
			sm_a[threadIdx.y + 0][threadIdx.x + ll * 16] = dev_t2[blk_idx_e * SIZE_SLICE_1_E + ll + (blk_idx_c * SIZE_SLICE_1_C + 0 + (blk_idx_b * SIZE_SLICE_1_B + 0 + ((blk_idx_a * SIZE_SLICE_1_A + idx_a) * size_f) * size_b) * size_c) * size_e + (threadIdx.y + l + 0) * stride_int_t2];
			// Exception: Temp. version!: threadIdx.y + l + 8
			// Exception: Temp. version!: 0 < rng_c
			// Exception: Full-Full
			sm_a[threadIdx.y + 8][threadIdx.x + ll * 16] = dev_t2[blk_idx_e * SIZE_SLICE_1_E + ll + (blk_idx_c * SIZE_SLICE_1_C + 0 + (blk_idx_b * SIZE_SLICE_1_B + 0 + ((blk_idx_a * SIZE_SLICE_1_A + idx_a) * size_f) * size_b) * size_c) * size_e + (threadIdx.y + l + 8) * stride_int_t2];
		}
		...
		for (int ll = 0; ll < 8; ll++)
		{
			...
			sm_b[threadIdx.x][threadIdx.y + ll * 8] = dev_v2[(blk_idx_d1 * SIZE_SLICE_1_D1 + idx_d1 + (blk_idx_d2 * SIZE_SLICE_1_D2 + ll) * size_d1) * size_f + (threadIdx.x + l)];
		}
		__syncthreads();
		//---------------------------------------------------------------------------------------------------
		...
		for (int ll = 0; ll < SIZE_INT_UNIT_1; ll++)
		{
			...
			{
				temp_av = sm_a[ll][idx_c + (idx_b + (idx_a) * SIZE_SLICE_1_B) * SIZE_SLICE_1_C + (xx * 16)];
...
int size_internal)
{
	...
	int rng_a, rng_b, rng_c, rng_d1, rng_d2, rng_e;
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		// This Part is for Loading Input-Left
		// tc_gen_code_Kernel_Load_Inputs_Abstracts()
		if (0 < rng_c && 0 < rng_b && idx_a < rng_a)
	...
	// Part: Generalized Threads
	// Part: Generalized Register-Tiling
	if (idx_a < rng_a && idx_c < rng_c && idx_b < rng_b && idx_d1 < rng_d1)
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 4; j++)
		{
...
int size_internal)
{
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		// This Part is for Loading Input-Left
		// tc_gen_code_Kernel_Load_Inputs_Abstracts()
		if (0 < rng_c && 0 < rng_b && idx_a < rng_a && threadIdx.y < SIZE_INT_UNIT_1 - internal_upperbound)
		for (int ll = 0; ll < rng_e; ll++)
		{
			// ['e', 'c', 'b', 'f', 'a']
			// Exception: Temp. version!: threadIdx.y + l + 0
			...
			// Exception: Temp. version!: threadIdx.y + l + 8
			// Exception: Temp. version!: 0 < rng_c
			if (threadIdx.y + l + 8 < size_internal && 0 < rng_c) 
			sm_a[threadIdx.y + 8][threadIdx.x + ll * 16] = dev_t2[blk_idx_e * SIZE_SLICE_1_E + ll + (blk_idx_c * SIZE_SLICE_1_C + 0 + (blk_idx_b * SIZE_SLICE_1_B + 0 + ((blk_idx_a * SIZE_SLICE_1_A + idx_a) * size_f) * size_b) * size_c) * size_e + (threadIdx.y + l + 8) * stride_int_t2];
		}
...
extern "C"
void sd_t_d2_fusion(int size_a, int size_b, int size_c, int size_d1, int size_d2, int size_e, int size_f, double* t3, double* host_t2, double* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	num_thread_blocks_kernel_1 = CEIL(size_a, SIZE_SLICE_1_A) * CEIL(size_b, SIZE_SLICE_1_B) * CEIL(size_c, SIZE_SLICE_1_C) * CEIL(size_d1, SIZE_SLICE_1_D1) * CEIL(size_d2, SIZE_SLICE_1_D2) * CEIL(size_e, SIZE_SLICE_1_E);
	// cudaMalloc()
	cudaMalloc((void**) &dev_t3, sizeof(double) * size_a * size_b * size_c * size_d1 * size_d2 * size_e);
	cudaMalloc((void**) &dev_t2, sizeof(double) * size_a * size_f * size_b * size_c * size_e);
	cudaMalloc((void**) &dev_v2, sizeof(double) * size_d2 * size_d1 * size_f);

	// cudaMemcpy()
	cudaMemcpy(dev_t3, t3, sizeof(double) * size_a * size_b * size_c * size_d1 * size_d2 * size_e, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_a * size_f * size_b * size_c * size_e, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_d2 * size_d1 * size_f, cudaMemcpyHostToDevice);

	// Related to Kernels
	// There are 1 Basic Kernels
	long long int tmp_operations = 2 * (long long int)(size_a * size_b * size_c * size_d1 * size_d2 * size_e) * size_f;
	printf ("========================================= fusedKernels =============================================\n");
	...
	int stride_output_d1 = stride_output_c * size_c;
	int stride_output_d2 = stride_output_d1 * size_d1;
	int stride_output_e = stride_output_d2 * size_d2;
	...
	int stride_int_t2 = size_e * size_c * size_b;
	int stride_int_v2 = 1;

	// Decision Tree for Kernel Types
	// No Chance to Utilize the Register Transpose
	if (size_a % SIZE_SLICE_1_A == 0 && size_b % SIZE_SLICE_1_B == 0 && size_c % SIZE_SLICE_1_C == 0 && size_d1 % SIZE_SLICE_1_D1 == 0 && size_d2 % SIZE_SLICE_1_D2 == 0 && size_e % SIZE_SLICE_1_E == 0)
	{
		...
		if (size_f % SIZE_SLICE_1_F == 0)
		{
			...
			kernel__1_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b, size_c, size_d1, size_d2, size_e, size_f, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b, SIZE_SLICE_1_B), CEIL(size_c, SIZE_SLICE_1_C), CEIL(size_d1, SIZE_SLICE_1_D1), CEIL(size_d2, SIZE_SLICE_1_D2), CEIL(size_e, SIZE_SLICE_1_E), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
		}
		else
		{
			...
			kernel__2_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b, size_c, size_d1, size_d2, size_e, size_f, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b, SIZE_SLICE_1_B), CEIL(size_c, SIZE_SLICE_1_C), CEIL(size_d1, SIZE_SLICE_1_D1), CEIL(size_d2, SIZE_SLICE_1_D2), CEIL(size_e, SIZE_SLICE_1_E), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
		}
	}
	else
	{
		...
		if (size_f % SIZE_SLICE_1_F == 0)
		{
			...
			kernel__3_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b, size_c, size_d1, size_d2, size_e, size_f, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b, SIZE_SLICE_1_B), CEIL(size_c, SIZE_SLICE_1_C), CEIL(size_d1, SIZE_SLICE_1_D1), CEIL(size_d2, SIZE_SLICE_1_D2), CEIL(size_e, SIZE_SLICE_1_E), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
		}
		else
		{
			// [4] Internal Index: Partial
			...
			kernel__4_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b, size_c, size_d1, size_d2, size_e, size_f, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b, SIZE_SLICE_1_B), CEIL(size_c, SIZE_SLICE_1_C), CEIL(size_d1, SIZE_SLICE_1_D1), CEIL(size_d2, SIZE_SLICE_1_D2), CEIL(size_e, SIZE_SLICE_1_E), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
		}
	}

	// Copy the Result from Device to Host
	cudaMemcpy(t3, dev_t3, sizeof(double) * (size_a * size_b * size_c * size_d1 * size_d2 * size_e), cudaMemcpyDeviceToHost);
...
void sd_t_d2_fusion_(int size_a, int size_b, int size_c, int size_d, int size_e, int size_f, double* t3, double* t2, double* v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	// Call An Application
	sd_t_d2_fusion(size_a, size_b, size_c, size_d1, size_d2, size_e, size_f, t3, t2, v2, cond_kernel_1, opt_register_transpose);
}
