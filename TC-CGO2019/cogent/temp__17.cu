...
int size_internal)
{
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['a', 'd', 'c']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['b1', 'b2', 'd']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			sm_a[threadIdx.y][threadIdx.x + ll * 16] = dev_t2[blk_idx_a * SIZE_SLICE_1_A + idx_a + ((blk_idx_c * SIZE_SLICE_1_C + ll) * size_d) * size_a + (threadIdx.y + l) * stride_int_t2];
		}
