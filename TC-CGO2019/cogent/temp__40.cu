...
float* dev_t2, 
float* dev_v2, 
int size_a, int size_f1, int size_f2, int size_b, int size_c, int size_g, 
int numBlk_a, int numBlk_f1, int numBlk_f2, int numBlk_b, int numBlk_c, 
...
int size_internal)
{
	...
	int tmp_blkIdx;
	int blk_idx_c = blockIdx.x / (numBlk_b * numBlk_f2 * numBlk_f1 * numBlk_a);
	tmp_blkIdx = blockIdx.x % (numBlk_b * numBlk_f2 * numBlk_f1 * numBlk_a);
	...
	int t3_base_thread = blk_idx_a * SIZE_SLICE_1_A + idx_a + (blk_idx_f1 * SIZE_SLICE_1_F1 + idx_f1 + (blk_idx_f2 * SIZE_SLICE_1_F2 + (blk_idx_b * SIZE_SLICE_1_B + (blk_idx_c * SIZE_SLICE_1_C + idx_c) * size_b) * size_f2) * size_f1) * size_a;
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['g', 'a', 'b', 'c']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['g', 'f1', 'f2']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 4; ll++)
		{
			...
			// Exception: Temp. version!: idx_f1 + 0 < rng_a
			sm_a[threadIdx.x][threadIdx.y + 0 + ll * 16] = dev_t2[(blk_idx_a * SIZE_SLICE_1_A + idx_f1 + 0 + (blk_idx_b * SIZE_SLICE_1_B + ll + (blk_idx_c * SIZE_SLICE_1_C + 0) * size_b) * size_a) * size_g + (threadIdx.x + l)];
			// Exception: Temp. version!: threadIdx.x + l
			// Exception: Temp. version!: idx_f1 + 8 < rng_a
			// Exception: Full-Full
			sm_a[threadIdx.x][threadIdx.y + 8 + ll * 16] = dev_t2[(blk_idx_a * SIZE_SLICE_1_A + idx_f1 + 8 + (blk_idx_b * SIZE_SLICE_1_B + ll + (blk_idx_c * SIZE_SLICE_1_C + 0) * size_b) * size_a) * size_g + (threadIdx.x + l)];
		}
...
int size_internal)
{
	...
	int rng_a, rng_f1, rng_f2, rng_b, rng_c;
	...
	// Part: Generalized Threads
	// Part: Generalized Register-Tiling
	if (idx_a < rng_a && idx_c < rng_c && idx_f1 < rng_f1)
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 4; j++)
		{
...
int size_internal)
{
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		// This Part is for Loading Input-Left
		// tc_gen_code_Kernel_Load_Inputs_Abstracts()
		if (idx_f1 < rng_a && 0 < rng_c && threadIdx.x < SIZE_INT_UNIT_1 - internal_upperbound)
		for (int ll = 0; ll < rng_b; ll++)
		{
			// ['g', 'a', 'b', 'c']
			// Exception: Temp. version!: threadIdx.x + l
			...
			// Exception: Temp. version!: threadIdx.x + l
			// Exception: Temp. version!: idx_f1 + 8 < rng_a
			if (threadIdx.x + l < size_internal && idx_f1 + 8 < rng_a) 
			sm_a[threadIdx.x][threadIdx.y + 8 + ll * 16] = dev_t2[(blk_idx_a * SIZE_SLICE_1_A + idx_f1 + 8 + (blk_idx_b * SIZE_SLICE_1_B + ll + (blk_idx_c * SIZE_SLICE_1_C + 0) * size_b) * size_a) * size_g + (threadIdx.x + l)];
		}
		
		// This Part is for Loading Input-Right
		// tc_gen_code_Kernel_Load_Inputs_Abstracts()
		if (idx_f1 < rng_f1 && threadIdx.x < SIZE_INT_UNIT_1 - internal_upperbound)
		for (int ll = 0; ll < rng_f2; ll++)
		{
			// ['g', 'f1', 'f2']
			// Exception: Temp. version!: threadIdx.x + l
...
extern "C"
void sd_t_d2_fusion(int size_a, int size_f1, int size_f2, int size_b, int size_c, int size_g, float* t3, float* host_t2, float* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	num_thread_blocks_kernel_1 = CEIL(size_a, SIZE_SLICE_1_A) * CEIL(size_f1, SIZE_SLICE_1_F1) * CEIL(size_f2, SIZE_SLICE_1_F2) * CEIL(size_b, SIZE_SLICE_1_B) * CEIL(size_c, SIZE_SLICE_1_C);
	// cudaMalloc()
	cudaMalloc((void**) &dev_t3, sizeof(float) * 4 * size_a * size_f1 * size_f2 * size_b * size_c);
	cudaMalloc((void**) &dev_t2, sizeof(float) * 4 *size_c * size_b * size_a * size_g);
	cudaMalloc((void**) &dev_v2, sizeof(float) * 4 *size_f2 * size_f1 * size_g);

	// // cudaMemcpy()
	// cudaMemcpy(dev_t3, t3, sizeof(float) * size_a * size_f1 * size_f2 * size_b * size_c, cudaMemcpyHostToDevice);
	// cudaMemcpy(dev_t2, host_t2, sizeof(float) * size_c * size_b * size_a * size_g, cudaMemcpyHostToDevice);
	// cudaMemcpy(dev_v2, host_v2, sizeof(float) * size_f2 * size_f1 * size_g, cudaMemcpyHostToDevice);

	// Related to Kernels
	// There are 1 Basic Kernels
	long long int tmp_operations = (long long int)(size_a * size_f1 * size_f2 * size_b * size_c) * size_g;
	printf ("========================================= fusedKernels =============================================\n");
	...
	if (size_a % SIZE_SLICE_1_A == 0 && size_f1 % SIZE_SLICE_1_F1 == 0 && size_f2 % SIZE_SLICE_1_F2 == 0 && size_b % SIZE_SLICE_1_B == 0 && size_c % SIZE_SLICE_1_C == 0)
	{
		...
		if (size_g % SIZE_SLICE_1_G == 0)
		{
			...
			kernel__1_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_f1, size_f2, size_b, size_c, size_g, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_f1, SIZE_SLICE_1_F1), CEIL(size_f2, SIZE_SLICE_1_F2), CEIL(size_b, SIZE_SLICE_1_B), CEIL(size_c, SIZE_SLICE_1_C), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
			kernel__1_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_f1, size_f2, size_b, size_c, size_g, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_f1, SIZE_SLICE_1_F1), CEIL(size_f2, SIZE_SLICE_1_F2), CEIL(size_b, SIZE_SLICE_1_B), CEIL(size_c, SIZE_SLICE_1_C), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
			kernel__1_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_f1, size_f2, size_b, size_c, size_g, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_f1, SIZE_SLICE_1_F1), CEIL(size_f2, SIZE_SLICE_1_F2), CEIL(size_b, SIZE_SLICE_1_B), CEIL(size_c, SIZE_SLICE_1_C), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
			kernel__1_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_f1, size_f2, size_b, size_c, size_g, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_f1, SIZE_SLICE_1_F1), CEIL(size_f2, SIZE_SLICE_1_F2), CEIL(size_b, SIZE_SLICE_1_B), CEIL(size_c, SIZE_SLICE_1_C), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
			kernel__1_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_f1, size_f2, size_b, size_c, size_g, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_f1, SIZE_SLICE_1_F1), CEIL(size_f2, SIZE_SLICE_1_F2), CEIL(size_b, SIZE_SLICE_1_B), CEIL(size_c, SIZE_SLICE_1_C), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
		...
		else
		{
			...
			kernel__2_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_f1, size_f2, size_b, size_c, size_g, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_f1, SIZE_SLICE_1_F1), CEIL(size_f2, SIZE_SLICE_1_F2), CEIL(size_b, SIZE_SLICE_1_B), CEIL(size_c, SIZE_SLICE_1_C), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
		}
	}
	else
	{
		...
		if (size_g % SIZE_SLICE_1_G == 0)
		{
			...
			kernel__3_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_f1, size_f2, size_b, size_c, size_g, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_f1, SIZE_SLICE_1_F1), CEIL(size_f2, SIZE_SLICE_1_F2), CEIL(size_b, SIZE_SLICE_1_B), CEIL(size_c, SIZE_SLICE_1_C), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
		}
		else
		{
			// [4] Internal Index: Partial
			...
			kernel__4_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_f1, size_f2, size_b, size_c, size_g, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_f1, SIZE_SLICE_1_F1), CEIL(size_f2, SIZE_SLICE_1_F2), CEIL(size_b, SIZE_SLICE_1_B), CEIL(size_c, SIZE_SLICE_1_C), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
		}
	}

	// Copy the Result from Device to Host
	// cudaMemcpy(t3, dev_t3, sizeof(float) * (size_a * size_f1 * size_f2 * size_b * size_c), cudaMemcpyDeviceToHost);
...
extern "C"
void sd_t_d2_fusion_(int size_a, int size_f, int size_b, int size_c, int size_g, float* t3, float* t2, float* v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	// Call An Application
	sd_t_d2_fusion(size_a, size_f1, size_f2, size_b, size_c, size_g, t3, t2, v2, cond_kernel_1, opt_register_transpose);
}
