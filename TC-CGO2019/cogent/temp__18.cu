...
int size_internal)
{
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['a', 'd', 'c']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['d', 'b1', 'b2']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 4; ll++)
		{
			...
			sm_a[threadIdx.y + 0][threadIdx.x + ll * 16] = dev_t2[blk_idx_a * SIZE_SLICE_1_A + idx_a + ((blk_idx_c * SIZE_SLICE_1_C + ll) * size_d) * size_a + (threadIdx.y + l + 0) * stride_int_t2];
			// Exception: Temp. version!: threadIdx.y + l + 8
			// Exception: Temp. version!: idx_a < rng_a
			// Exception: Full-Full
			sm_a[threadIdx.y + 8][threadIdx.x + ll * 16] = dev_t2[blk_idx_a * SIZE_SLICE_1_A + idx_a + ((blk_idx_c * SIZE_SLICE_1_C + ll) * size_d) * size_a + (threadIdx.y + l + 8) * stride_int_t2];
		}
...
void sd_t_d2_fusion(int size_a, int size_b1, int size_b2, int size_c, int size_d, double* t3, double* host_t2, double* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	cudaMalloc((void**) &dev_t2, sizeof(double) * size_c * size_d * size_a);
	...
	cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_c * size_d * size_a, cudaMemcpyHostToDevice);
