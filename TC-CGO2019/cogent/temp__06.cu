...
#define SIZE_TB_1_X 	SIZE_SLICE_1_A * SIZE_SLICE_1_B * SIZE_SLICE_1_D
...
double* dev_t2, 
double* dev_v2, 
int size_a, int size_b, int size_c1, int size_c2, int size_d, int size_e, int size_f, 
int numBlk_a, int numBlk_b, int numBlk_c1, int numBlk_c2, int numBlk_d, int numBlk_e, 
...
int size_internal)
{
	...
	int idx_d = threadIdx.x / (SIZE_SLICE_1_B * SIZE_SLICE_1_A);
	int remaining_idx = threadIdx.x % (SIZE_SLICE_1_B * SIZE_SLICE_1_A);
	int idx_b = remaining_idx / (SIZE_SLICE_1_A);
	remaining_idx = remaining_idx % (SIZE_SLICE_1_A);
	int idx_a = remaining_idx;
	int idx_c1 = threadIdx.y;

	int tmp_blkIdx;
	int blk_idx_e = blockIdx.x / (numBlk_d * numBlk_c2 * numBlk_c1 * numBlk_b * numBlk_a);
	tmp_blkIdx = blockIdx.x % (numBlk_d * numBlk_c2 * numBlk_c1 * numBlk_b * numBlk_a);

	int blk_idx_d = tmp_blkIdx / (numBlk_c2 * numBlk_c1 * numBlk_b * numBlk_a);
	tmp_blkIdx = tmp_blkIdx % (numBlk_c2 * numBlk_c1 * numBlk_b * numBlk_a);
	...
	int t3_base_thread = blk_idx_a * SIZE_SLICE_1_A + idx_a + (blk_idx_b * SIZE_SLICE_1_B + idx_b + (blk_idx_c1 * SIZE_SLICE_1_C1 + idx_c1 + (blk_idx_c2 * SIZE_SLICE_1_C2 + (blk_idx_d * SIZE_SLICE_1_D + idx_d + (blk_idx_e * SIZE_SLICE_1_E) * size_d) * size_c2) * size_c1) * size_b) * size_a;
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['e', 'f', 'b', 'a', 'd']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['c1', 'c2', 'f']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 4; ll++)
		{
			...
			sm_a[threadIdx.y][threadIdx.x + ll * 16] = dev_t2[blk_idx_e * SIZE_SLICE_1_E + ll + ((blk_idx_b * SIZE_SLICE_1_B + 0 + (blk_idx_a * SIZE_SLICE_1_A + idx_a + (blk_idx_d * SIZE_SLICE_1_D + 0) * size_a) * size_b) * size_f) * size_e + (threadIdx.y + l) * stride_int_t2];
		}
		...
		for (int ll = 0; ll < 4; ll++)
		{
			// ['c1', 'c2', 'f']
			// Exception: Temp. version!: threadIdx.y + l
			// Exception: Temp. version!: idx_a < rng_c1
			sm_b[threadIdx.y][threadIdx.x + ll * 16] = dev_v2[blk_idx_c1 * SIZE_SLICE_1_C1 + idx_a + (blk_idx_c2 * SIZE_SLICE_1_C2 + ll) * size_c1 + (threadIdx.y + l) * stride_int_v2];
		}
		__syncthreads();
		//---------------------------------------------------------------------------------------------------
		...
		for (int ll = 0; ll < SIZE_INT_UNIT_1; ll++)
		{
			...
			temp_bv[1] = sm_b[ll][idx_c1 + 16];
			temp_bv[2] = sm_b[ll][idx_c1 + 32];
			temp_bv[3] = sm_b[ll][idx_c1 + 48];

			for (int xx = 0; xx < 4; xx++) // (1)
			{
				temp_av = sm_a[ll][idx_b + (idx_a + (idx_d) * SIZE_SLICE_1_A) * SIZE_SLICE_1_B + (xx * 16)];
...
int size_internal)
{
	...
	int rng_a, rng_b, rng_c1, rng_c2, rng_d, rng_e;
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		// This Part is for Loading Input-Left
		// tc_gen_code_Kernel_Load_Inputs_Abstracts()
		if (0 < rng_b && idx_a < rng_a && 0 < rng_d)
	...
	// Part: Generalized Threads
	// Part: Generalized Register-Tiling
	if (idx_a < rng_a && idx_b < rng_b && idx_d < rng_d && idx_c1 < rng_c1)
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 4; j++)
		{
...
int size_internal)
{
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		// This Part is for Loading Input-Left
		// tc_gen_code_Kernel_Load_Inputs_Abstracts()
		if (0 < rng_b && idx_a < rng_a && 0 < rng_d && threadIdx.y < SIZE_INT_UNIT_1 - internal_upperbound)
		for (int ll = 0; ll < rng_e; ll++)
		{
			// ['e', 'f', 'b', 'a', 'd']
			// Exception: Temp. version!: threadIdx.y + l
		...
		// This Part is for Loading Input-Right
		// tc_gen_code_Kernel_Load_Inputs_Abstracts()
		if (idx_a < rng_c1 && threadIdx.y < SIZE_INT_UNIT_1 - internal_upperbound)
		for (int ll = 0; ll < rng_c2; ll++)
		{
			// ['c1', 'c2', 'f']
			// Exception: Temp. version!: threadIdx.y + l
...
extern "C"
void sd_t_d2_fusion(int size_a, int size_b, int size_c1, int size_c2, int size_d, int size_e, int size_f, double* t3, double* host_t2, double* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	num_thread_blocks_kernel_1 = CEIL(size_a, SIZE_SLICE_1_A) * CEIL(size_b, SIZE_SLICE_1_B) * CEIL(size_c1, SIZE_SLICE_1_C1) * CEIL(size_c2, SIZE_SLICE_1_C2) * CEIL(size_d, SIZE_SLICE_1_D) * CEIL(size_e, SIZE_SLICE_1_E);
	// cudaMalloc()
	cudaMalloc((void**) &dev_t3, sizeof(double) * size_a * size_b * size_c1 * size_c2 * size_d * size_e);
	cudaMalloc((void**) &dev_t2, sizeof(double) * size_d * size_a * size_b * size_f * size_e);
	cudaMalloc((void**) &dev_v2, sizeof(double) * size_f * size_c2 * size_c1);

	// cudaMemcpy()
	cudaMemcpy(dev_t3, t3, sizeof(double) * size_a * size_b * size_c1 * size_c2 * size_d * size_e, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_d * size_a * size_b * size_f * size_e, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_f * size_c2 * size_c1, cudaMemcpyHostToDevice);

	// Related to Kernels
	// There are 1 Basic Kernels
	long long int tmp_operations = 2 * (long long int)(size_a * size_b * size_c1 * size_c2 * size_d * size_e) * size_f;
	printf ("========================================= fusedKernels =============================================\n");
	...
	int stride_int_v2 = size_c1 * size_c2;

	// Decision Tree for Kernel Types
	// No Chance to Utilize the Register Transpose
	if (size_a % SIZE_SLICE_1_A == 0 && size_b % SIZE_SLICE_1_B == 0 && size_c1 % SIZE_SLICE_1_C1 == 0 && size_c2 % SIZE_SLICE_1_C2 == 0 && size_d % SIZE_SLICE_1_D == 0 && size_e % SIZE_SLICE_1_E == 0)
	{
		...
		if (size_f % SIZE_SLICE_1_F == 0)
		{
			...
			kernel__1_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b, size_c1, size_c2, size_d, size_e, size_f, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b, SIZE_SLICE_1_B), CEIL(size_c1, SIZE_SLICE_1_C1), CEIL(size_c2, SIZE_SLICE_1_C2), CEIL(size_d, SIZE_SLICE_1_D), CEIL(size_e, SIZE_SLICE_1_E), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
		}
		else
		{
			...
			kernel__2_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b, size_c1, size_c2, size_d, size_e, size_f, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b, SIZE_SLICE_1_B), CEIL(size_c1, SIZE_SLICE_1_C1), CEIL(size_c2, SIZE_SLICE_1_C2), CEIL(size_d, SIZE_SLICE_1_D), CEIL(size_e, SIZE_SLICE_1_E), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
		}
	}
	else
	{
		...
		if (size_f % SIZE_SLICE_1_F == 0)
		{
			...
			kernel__3_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b, size_c1, size_c2, size_d, size_e, size_f, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b, SIZE_SLICE_1_B), CEIL(size_c1, SIZE_SLICE_1_C1), CEIL(size_c2, SIZE_SLICE_1_C2), CEIL(size_d, SIZE_SLICE_1_D), CEIL(size_e, SIZE_SLICE_1_E), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
		}
		else
		{
			// [4] Internal Index: Partial
			...
			kernel__4_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b, size_c1, size_c2, size_d, size_e, size_f, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b, SIZE_SLICE_1_B), CEIL(size_c1, SIZE_SLICE_1_C1), CEIL(size_c2, SIZE_SLICE_1_C2), CEIL(size_d, SIZE_SLICE_1_D), CEIL(size_e, SIZE_SLICE_1_E), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
		}
	}

	// Copy the Result from Device to Host
	cudaMemcpy(t3, dev_t3, sizeof(double) * (size_a * size_b * size_c1 * size_c2 * size_d * size_e), cudaMemcpyDeviceToHost);
...
void sd_t_d2_fusion_(int size_a, int size_b, int size_c, int size_d, int size_e, int size_f, double* t3, double* t2, double* v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	// Call An Application
	sd_t_d2_fusion(size_a, size_b, size_c1, size_c2, size_d, size_e, size_f, t3, t2, v2, cond_kernel_1, opt_register_transpose);
}
