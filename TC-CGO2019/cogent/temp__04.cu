...
int size_internal)
{
	...
	int t3_base_thread = blk_idx_a * SIZE_SLICE_1_A + idx_a + (blk_idx_b1 * SIZE_SLICE_1_B1 + idx_b1 + (blk_idx_b2 * SIZE_SLICE_1_B2 + (blk_idx_c * SIZE_SLICE_1_C + idx_c + (blk_idx_d * SIZE_SLICE_1_D) * size_c) * size_b2) * size_b1) * size_a;
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['d', 'e', 'c', 'a']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['b1', 'b2', 'e']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			sm_a[threadIdx.y][threadIdx.x + ll * 16] = dev_t2[blk_idx_d * SIZE_SLICE_1_D + ll + ((blk_idx_c * SIZE_SLICE_1_C + 0 + (blk_idx_a * SIZE_SLICE_1_A + idx_a) * size_c) * size_e) * size_d + (threadIdx.y + l) * stride_int_t2];
		}
...
int size_internal)
{
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		// This Part is for Loading Input-Left
		// tc_gen_code_Kernel_Load_Inputs_Abstracts()
		if (0 < rng_c && idx_a < rng_a && threadIdx.y < SIZE_INT_UNIT_1 - internal_upperbound)
		for (int ll = 0; ll < rng_d; ll++)
		{
			// ['d', 'e', 'c', 'a']
			// Exception: Temp. version!: threadIdx.y + l
...
void sd_t_d2_fusion(int size_a, int size_b1, int size_b2, int size_c, int size_d, int size_e, double* t3, double* host_t2, double* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	cudaMalloc((void**) &dev_t2, sizeof(double) * size_a * size_c * size_e * size_d);
	cudaMalloc((void**) &dev_v2, sizeof(double) * size_e * size_b2 * size_b1);

	// cudaMemcpy()
	cudaMemcpy(dev_t3, t3, sizeof(double) * size_a * size_b1 * size_b2 * size_c * size_d, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_a * size_c * size_e * size_d, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_e * size_b2 * size_b1, cudaMemcpyHostToDevice);
