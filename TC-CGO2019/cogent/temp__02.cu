...
int size_internal)
{
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['d', 'c', 'a']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['b1', 'b2', 'd']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 4; ll++)
		{
			...
			// Exception: Temp. version!: idx_b1 < rng_a
			sm_a[threadIdx.x][threadIdx.y + ll * 16] = dev_t2[(blk_idx_c * SIZE_SLICE_1_C + ll + (blk_idx_a * SIZE_SLICE_1_A + idx_b1) * size_c) * size_d + (threadIdx.x + l)];
		}
...
int size_internal)
{
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		// This Part is for Loading Input-Left
		// tc_gen_code_Kernel_Load_Inputs_Abstracts()
		if (idx_b1 < rng_a && threadIdx.x < SIZE_INT_UNIT_1 - internal_upperbound)
		for (int ll = 0; ll < rng_c; ll++)
		{
			// ['d', 'c', 'a']
			// Exception: Temp. version!: threadIdx.x + l
...
void sd_t_d2_fusion(int size_a, int size_b1, int size_b2, int size_c, int size_d, double* t3, double* host_t2, double* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	cudaMalloc((void**) &dev_t2, sizeof(double) * size_a * size_c * size_d);
	...
	cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_a * size_c * size_d, cudaMemcpyHostToDevice);
