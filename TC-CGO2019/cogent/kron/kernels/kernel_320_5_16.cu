...
int size_internal)
{
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 4; ll++)
		{
			...
			sm_a[threadIdx.y + 0][threadIdx.x + ll * 16] = dev_t2[blk_idx_a * SIZE_SLICE_1_A + idx_a + (blk_idx_b * SIZE_SLICE_1_B + ll + (blk_idx_c * SIZE_SLICE_1_C + 0 + (blk_idx_d * SIZE_SLICE_1_D + 0 + (blk_idx_e * SIZE_SLICE_1_E + 0) * size_d) * size_c) * size_b) * size_a + (threadIdx.y + l + 0) * stride_int_t2];
			// Exception: Temp. version!: threadIdx.y + l + 8
			// Exception: Temp. version!: idx_a < rng_a
			// Exception: Full-Full
			sm_a[threadIdx.y + 8][threadIdx.x + ll * 16] = dev_t2[blk_idx_a * SIZE_SLICE_1_A + idx_a + (blk_idx_b * SIZE_SLICE_1_B + ll + (blk_idx_c * SIZE_SLICE_1_C + 0 + (blk_idx_d * SIZE_SLICE_1_D + 0 + (blk_idx_e * SIZE_SLICE_1_E + 0) * size_d) * size_c) * size_b) * size_a + (threadIdx.y + l + 8) * stride_int_t2];
		}
