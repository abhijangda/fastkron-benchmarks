...
int size_internal)
{
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['a1', 'a2', 'c', 'd']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['d', 'b1', 'b2', 'c']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 4; ll++)
		{
			...
			sm_a[threadIdx.y + 0][threadIdx.x + ll * 16] = dev_t2[blk_idx_a1 * SIZE_SLICE_1_A1 + idx_a1 + (blk_idx_a2 * SIZE_SLICE_1_A2 + ll) * size_a1 + const_internal_t2_offset[threadIdx.y + l + 0]];
			// Exception: Temp. version!: threadIdx.y + l + 8
			// Exception: Temp. version!: idx_a1 < rng_a1
			// Exception: Full-Full
			sm_a[threadIdx.y + 8][threadIdx.x + ll * 16] = dev_t2[blk_idx_a1 * SIZE_SLICE_1_A1 + idx_a1 + (blk_idx_a2 * SIZE_SLICE_1_A2 + ll) * size_a1 + const_internal_t2_offset[threadIdx.y + l + 8]];
		}
		...
		for (int ll = 0; ll < 8; ll++)
		{
			...
			sm_b[threadIdx.y + 0][threadIdx.x + ll * 8] = dev_v2[(blk_idx_b1 * SIZE_SLICE_1_B1 + idx_a1 + (blk_idx_b2 * SIZE_SLICE_1_B2 + ll) * size_b1) * size_d + const_internal_v2_offset[threadIdx.y + l + 0]];
			// Exception: Temp. version!: threadIdx.y + l + 8
			// Exception: Temp. version!: idx_a1 < rng_b1
			// Exception: Full-Full
			sm_b[threadIdx.y + 8][threadIdx.x + ll * 8] = dev_v2[(blk_idx_b1 * SIZE_SLICE_1_B1 + idx_a1 + (blk_idx_b2 * SIZE_SLICE_1_B2 + ll) * size_b1) * size_d + const_internal_v2_offset[threadIdx.y + l + 8]];
		}
		__syncthreads();
		//---------------------------------------------------------------------------------------------------
...
int size_internal)
{
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 4; ll++)
		{
			...
			// Exception: Temp. version!: threadIdx.y + l + 0
			// Exception: Temp. version!: idx_a1 < rng_a1
			sm_a[threadIdx.y + 0][threadIdx.x + ll * 16] = dev_t2[blk_idx_a1 * SIZE_SLICE_1_A1 + idx_a1 + (blk_idx_a2 * SIZE_SLICE_1_A2 + ll) * size_a1 + dev_internal_offset_t2[threadIdx.y + l + 0]];
			// Exception: Temp. version!: threadIdx.y + l + 8
			// Exception: Temp. version!: idx_a1 < rng_a1
			// Exception: Full-Full
			sm_a[threadIdx.y + 8][threadIdx.x + ll * 16] = dev_t2[blk_idx_a1 * SIZE_SLICE_1_A1 + idx_a1 + (blk_idx_a2 * SIZE_SLICE_1_A2 + ll) * size_a1 + dev_internal_offset_t2[threadIdx.y + l + 8]];
		}
		...
		for (int ll = 0; ll < 8; ll++)
		{
			...
			// Exception: Temp. version!: threadIdx.y + l + 0
			// Exception: Temp. version!: idx_a1 < rng_b1
			sm_b[threadIdx.y + 0][threadIdx.x + ll * 8] = dev_v2[(blk_idx_b1 * SIZE_SLICE_1_B1 + idx_a1 + (blk_idx_b2 * SIZE_SLICE_1_B2 + ll) * size_b1) * size_d + dev_internal_offset_v2[threadIdx.y + l + 0]];
			// Exception: Temp. version!: threadIdx.y + l + 8
			// Exception: Temp. version!: idx_a1 < rng_b1
			// Exception: Full-Full
			sm_b[threadIdx.y + 8][threadIdx.x + ll * 8] = dev_v2[(blk_idx_b1 * SIZE_SLICE_1_B1 + idx_a1 + (blk_idx_b2 * SIZE_SLICE_1_B2 + ll) * size_b1) * size_d + dev_internal_offset_v2[threadIdx.y + l + 8]];
		}
		__syncthreads();
		//---------------------------------------------------------------------------------------------------
...
void sd_t_d2_fusion(int size_a1, int size_a2, int size_b1, int size_b2, int size_c, int size_d, double* t3, double* host_t2, double* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	cudaMalloc((void**) &dev_t2, sizeof(double) * size_d * size_c * size_a2 * size_a1);
	cudaMalloc((void**) &dev_v2, sizeof(double) * size_c * size_b2 * size_b1 * size_d);

	// cudaMemcpy()
	cudaMemcpy(dev_t3, t3, sizeof(double) * size_a1 * size_a2 * size_b1 * size_b2, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_d * size_c * size_a2 * size_a1, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_c * size_b2 * size_b1 * size_d, cudaMemcpyHostToDevice);
	...
	{
		host_internal_left_offset[idx_d + (idx_c) * size_d] 	= ((idx_c + (idx_d) * size_c) * size_a2) * size_a1;
		host_internal_right_offset[idx_d + (idx_c) * size_d] 	= idx_d + (((idx_c) * size_b2) * size_b1) * size_d;
	}
