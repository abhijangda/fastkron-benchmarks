...
int size_internal)
{
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['a', 'e', 'c', 'f']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['f', 'b', 'e', 'd']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			sm_a[threadIdx.y][threadIdx.x + ll * 16] = dev_t2[blk_idx_a * SIZE_SLICE_1_A + idx_a + ((blk_idx_c * SIZE_SLICE_1_C + ll) * size_e) * size_a + const_internal_t2_offset[threadIdx.y + l]];
		}
...
int size_internal)
{
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			// Exception: Temp. version!: threadIdx.y + l
			// Exception: Temp. version!: idx_a < rng_a
			sm_a[threadIdx.y][threadIdx.x + ll * 16] = dev_t2[blk_idx_a * SIZE_SLICE_1_A + idx_a + ((blk_idx_c * SIZE_SLICE_1_C + ll) * size_e) * size_a + dev_internal_offset_t2[threadIdx.y + l]];
		}
...
void sd_t_d2_fusion(int size_a, int size_b, int size_c, int size_d, int size_e, int size_f, double* t3, double* host_t2, double* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	cudaMalloc((void**) &dev_t2, sizeof(double) * size_f * size_c * size_e * size_a);
	...
	cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_f * size_c * size_e * size_a, cudaMemcpyHostToDevice);
	...
	{
		host_internal_left_offset[idx_f + (idx_e) * size_f] 	= (idx_e + ((idx_f) * size_c) * size_e) * size_a;
