...
int size_internal)
{
	...
	int t3_base_thread = blk_idx_a * SIZE_SLICE_1_A + idx_a + (blk_idx_b * SIZE_SLICE_1_B + (blk_idx_c1 * SIZE_SLICE_1_C1 + idx_c1 + (blk_idx_c2 * SIZE_SLICE_1_C2 + (blk_idx_d * SIZE_SLICE_1_D + idx_d) * size_c2) * size_c1) * size_b) * size_a;
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['e', 'b', 'a', 'd']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['c1', 'c2', 'e']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 4; ll++)
		{
			...
			// Exception: Temp. version!: idx_c1 < rng_a
			sm_a[threadIdx.x][threadIdx.y + ll * 16] = dev_t2[(blk_idx_b * SIZE_SLICE_1_B + ll + (blk_idx_a * SIZE_SLICE_1_A + idx_c1 + (blk_idx_d * SIZE_SLICE_1_D + 0) * size_a) * size_b) * size_e + (threadIdx.x + l)];
		}
		...
		for (int ll = 0; ll < SIZE_INT_UNIT_1; ll++)
		{
			...
			{
				temp_av = sm_a[ll][idx_a + (idx_d) * SIZE_SLICE_1_A + (xx * 16)];
...
int size_internal)
{
	...
	// Part: Generalized Threads
	// Part: Generalized Register-Tiling
	if (idx_a < rng_a && idx_d < rng_d && idx_c1 < rng_c1)
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 4; j++)
		{
...
int size_internal)
{
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		// This Part is for Loading Input-Left
		// tc_gen_code_Kernel_Load_Inputs_Abstracts()
		if (idx_c1 < rng_a && 0 < rng_d && threadIdx.x < SIZE_INT_UNIT_1 - internal_upperbound)
		for (int ll = 0; ll < rng_b; ll++)
		{
			// ['e', 'b', 'a', 'd']
			// Exception: Temp. version!: threadIdx.x + l
...
void sd_t_d2_fusion(int size_a, int size_b, int size_c1, int size_c2, int size_d, int size_e, double* t3, double* host_t2, double* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	cudaMalloc((void**) &dev_t2, sizeof(double) * size_d * size_a * size_b * size_e);
	cudaMalloc((void**) &dev_v2, sizeof(double) * size_e * size_c2 * size_c1);

	// cudaMemcpy()
	cudaMemcpy(dev_t3, t3, sizeof(double) * size_a * size_b * size_c1 * size_c2 * size_d, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_d * size_a * size_b * size_e, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_e * size_c2 * size_c1, cudaMemcpyHostToDevice);
