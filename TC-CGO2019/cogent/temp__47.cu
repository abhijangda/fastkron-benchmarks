...
int size_internal)
{
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['g', 'f', 'a', 'c']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['d', 'e', 'g', 'b']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 4; ll++)
		{
			...
			sm_a[threadIdx.x][threadIdx.y + 0 + ll * 16] = dev_t2[(blk_idx_f * SIZE_SLICE_1_F + ll + (blk_idx_a * SIZE_SLICE_1_A + idx_d + 0 + (blk_idx_c * SIZE_SLICE_1_C + 0) * size_a) * size_f) * size_g + (threadIdx.x + l)];
			// Exception: Temp. version!: threadIdx.x + l
			// Exception: Temp. version!: idx_d + 8 < rng_a
			// Exception: Full-Full
			sm_a[threadIdx.x][threadIdx.y + 8 + ll * 16] = dev_t2[(blk_idx_f * SIZE_SLICE_1_F + ll + (blk_idx_a * SIZE_SLICE_1_A + idx_d + 8 + (blk_idx_c * SIZE_SLICE_1_C + 0) * size_a) * size_f) * size_g + (threadIdx.x + l)];
		}
		...
		for (int ll = 0; ll < 8; ll++)
		{
			...
			sm_b[threadIdx.y][threadIdx.x + ll * 8] = dev_v2[blk_idx_d * SIZE_SLICE_1_D + idx_a + (blk_idx_e * SIZE_SLICE_1_E + ll + ((blk_idx_b * SIZE_SLICE_1_B + 0) * size_g) * size_e) * size_d + (threadIdx.y + l) * stride_int_v2];
		}
		__syncthreads();
		//---------------------------------------------------------------------------------------------------
		...
		{
			temp_bv[0] = sm_b[ll][idx_d + (idx_b) * SIZE_SLICE_1_D + 0];
			temp_bv[1] = sm_b[ll][idx_d + (idx_b) * SIZE_SLICE_1_D + 8];
			temp_bv[2] = sm_b[ll][idx_d + (idx_b) * SIZE_SLICE_1_D + 16];
			temp_bv[3] = sm_b[ll][idx_d + (idx_b) * SIZE_SLICE_1_D + 24];
			temp_bv[4] = sm_b[ll][idx_d + (idx_b) * SIZE_SLICE_1_D + 32];
			temp_bv[5] = sm_b[ll][idx_d + (idx_b) * SIZE_SLICE_1_D + 40];
			temp_bv[6] = sm_b[ll][idx_d + (idx_b) * SIZE_SLICE_1_D + 48];
			temp_bv[7] = sm_b[ll][idx_d + (idx_b) * SIZE_SLICE_1_D + 56];
...
void sd_t_d2_fusion(int size_a, int size_b, int size_c, int size_d, int size_e, int size_f, int size_g, double* t3, double* host_t2, double* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	cudaMalloc((void**) &dev_t2, sizeof(double) * size_c * size_a * size_f * size_g);
	cudaMalloc((void**) &dev_v2, sizeof(double) * size_b * size_g * size_e * size_d);

	// cudaMemcpy()
	cudaMemcpy(dev_t3, t3, sizeof(double) * size_a * size_b * size_c * size_d * size_e * size_f, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_c * size_a * size_f * size_g, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_b * size_g * size_e * size_d, cudaMemcpyHostToDevice);
