...
int size_internal)
{
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['a', 'e', 'd', 'f']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['b', 'f', 'c', 'e']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			sm_b[threadIdx.y][threadIdx.x + ll * 16] = dev_v2[blk_idx_b * SIZE_SLICE_1_B + idx_a + ((blk_idx_c * SIZE_SLICE_1_C + ll) * size_f) * size_b + const_internal_v2_offset[threadIdx.y + l]];
		}
		__syncthreads();
		//---------------------------------------------------------------------------------------------------
...
int size_internal)
{
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			// Exception: Temp. version!: threadIdx.y + l
			// Exception: Temp. version!: idx_a < rng_b
			sm_b[threadIdx.y][threadIdx.x + ll * 16] = dev_v2[blk_idx_b * SIZE_SLICE_1_B + idx_a + ((blk_idx_c * SIZE_SLICE_1_C + ll) * size_f) * size_b + dev_internal_offset_v2[threadIdx.y + l]];
		}
		__syncthreads();
		//---------------------------------------------------------------------------------------------------
...
void sd_t_d2_fusion(int size_a, int size_b, int size_c, int size_d, int size_e, int size_f, double* t3, double* host_t2, double* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	cudaMalloc((void**) &dev_v2, sizeof(double) * size_e * size_c * size_f * size_b);
	...
	cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_e * size_c * size_f * size_b, cudaMemcpyHostToDevice);
	...
	{
		host_internal_left_offset[idx_e + (idx_f) * size_e] 	= (idx_e + ((idx_f) * size_d) * size_e) * size_a;
		host_internal_right_offset[idx_e + (idx_f) * size_e] 	= (idx_f + ((idx_e) * size_c) * size_f) * size_b;
	}
