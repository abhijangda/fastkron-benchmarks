...
int size_internal)
{
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['a', 'e', 'b', 'f']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['f', 'd', 'e', 'c']], '+=']
	#pragma unroll 1
...
void sd_t_d2_fusion(int size_a, int size_b, int size_c, int size_d, int size_e, int size_f, double* t3, double* host_t2, double* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	cudaMalloc((void**) &dev_t2, sizeof(double) * size_f * size_b * size_e * size_a);
	...
	cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_f * size_b * size_e * size_a, cudaMemcpyHostToDevice);
	...
	{
		host_internal_left_offset[idx_f + (idx_e) * size_f] 	= (idx_e + ((idx_f) * size_b) * size_e) * size_a;
		host_internal_right_offset[idx_f + (idx_e) * size_f] 	= idx_f + ((idx_e) * size_d) * size_f;
	}
