...
#define SIZE_REG_1_X 	SIZE_SLICE_1_F
...
float* dev_t2, 
float* dev_v2, 
int size_a, int size_b1, int size_b2, int size_c, int size_f, int size_d, 
int numBlk_a, int numBlk_b1, int numBlk_b2, int numBlk_c, int numBlk_f, 
...
int size_internal)
{
	...
	int tmp_blkIdx;
	int blk_idx_f = blockIdx.x / (numBlk_c * numBlk_b2 * numBlk_b1 * numBlk_a);
	...
	int t3_base_thread = blk_idx_a * SIZE_SLICE_1_A + idx_a + (blk_idx_b1 * SIZE_SLICE_1_B1 + idx_b1 + (blk_idx_b2 * SIZE_SLICE_1_B2 + (blk_idx_c * SIZE_SLICE_1_C + idx_c + (blk_idx_f * SIZE_SLICE_1_F) * size_c) * size_b2) * size_b1) * size_a;
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['d', 'f', 'c', 'a']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['b1', 'b2', 'd']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 4; ll++)
		{
			...
			sm_a[threadIdx.x][threadIdx.y + ll * 16] = dev_t2[(blk_idx_f * SIZE_SLICE_1_F + ll + (blk_idx_c * SIZE_SLICE_1_C + 0 + (blk_idx_a * SIZE_SLICE_1_A + idx_b1) * size_c) * size_f) * size_d + (threadIdx.x + l)];
		}
		...
		for (int ll = 0; ll < SIZE_INT_UNIT_1; ll++)
		{
			...
			{
				temp_av = sm_a[ll][idx_c + (idx_a) * SIZE_SLICE_1_C + (xx * 16)];
...
int size_internal)
{
	...
	int rng_a, rng_b1, rng_b2, rng_c, rng_f;
	...
	// Part: Generalized Threads
	// Part: Generalized Register-Tiling
	if (idx_a < rng_a && idx_c < rng_c && idx_b1 < rng_b1)
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 4; j++)
		{
...
int size_internal)
{
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		// This Part is for Loading Input-Left
		// tc_gen_code_Kernel_Load_Inputs_Abstracts()
		if (0 < rng_c && idx_b1 < rng_a && threadIdx.x < SIZE_INT_UNIT_1 - internal_upperbound)
		for (int ll = 0; ll < rng_f; ll++)
		{
			// ['d', 'f', 'c', 'a']
			// Exception: Temp. version!: threadIdx.x + l
...
extern "C"
void sd_t_d2_fusion(int size_a, int size_b1, int size_b2, int size_c, int size_f, int size_d, float* t3, float* host_t2, float* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	num_thread_blocks_kernel_1 = CEIL(size_a, SIZE_SLICE_1_A) * CEIL(size_b1, SIZE_SLICE_1_B1) * CEIL(size_b2, SIZE_SLICE_1_B2) * CEIL(size_c, SIZE_SLICE_1_C) * CEIL(size_f, SIZE_SLICE_1_F);
	// cudaMalloc()
	cudaMalloc((void**) &dev_t3, sizeof(double) * size_a * size_b1 * size_b2 * size_c * size_f);
	cudaMalloc((void**) &dev_t2, sizeof(double) * size_a * size_c * size_f * size_d);
	cudaMalloc((void**) &dev_v2, sizeof(double) * size_d * size_b2 * size_b1);

	// cudaMemcpy()
	cudaMemcpy(dev_t3, t3, sizeof(double) * size_a * size_b1 * size_b2 * size_c * size_f, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_a * size_c * size_f * size_d, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_d * size_b2 * size_b1, cudaMemcpyHostToDevice);
	printf("sdfsdfs\n");
	// Related to Kernels
	// There are 1 Basic Kernels
	long long int tmp_operations = (long long int)(size_a * size_b1 * size_b2 * size_c * size_f) * size_d;
	printf ("========================================= fusedKernels =============================================\n");
	...
	if (size_a % SIZE_SLICE_1_A == 0 && size_b1 % SIZE_SLICE_1_B1 == 0 && size_b2 % SIZE_SLICE_1_B2 == 0 && size_c % SIZE_SLICE_1_C == 0 && size_f % SIZE_SLICE_1_F == 0)
	{
		...
		if (size_d % SIZE_SLICE_1_D == 0)
		{
			...
			kernel__1_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b1, size_b2, size_c, size_f, size_d, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b1, SIZE_SLICE_1_B1), CEIL(size_b2, SIZE_SLICE_1_B2), CEIL(size_c, SIZE_SLICE_1_C), CEIL(size_f, SIZE_SLICE_1_F), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
		}
		else
		{
			...
			kernel__2_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b1, size_b2, size_c, size_f, size_d, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b1, SIZE_SLICE_1_B1), CEIL(size_b2, SIZE_SLICE_1_B2), CEIL(size_c, SIZE_SLICE_1_C), CEIL(size_f, SIZE_SLICE_1_F), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
		}
	}
	else
	{
		...
		if (size_d % SIZE_SLICE_1_D == 0)
		{
			...
			kernel__3_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b1, size_b2, size_c, size_f, size_d, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b1, SIZE_SLICE_1_B1), CEIL(size_b2, SIZE_SLICE_1_B2), CEIL(size_c, SIZE_SLICE_1_C), CEIL(size_f, SIZE_SLICE_1_F), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
		...
		else
		{
			...
			kernel__4_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b1, size_b2, size_c, size_f, size_d, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b1, SIZE_SLICE_1_B1), CEIL(size_b2, SIZE_SLICE_1_B2), CEIL(size_c, SIZE_SLICE_1_C), CEIL(size_f, SIZE_SLICE_1_F), stride_int_t2, stride_int_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
		}
	}
	// Copy the Result from Device to Host
	// cudaMemcpy(t3, dev_t3, sizeof(float) * (size_a * size_b1 * size_b2 * size_c * size_f), cudaMemcpyDeviceToHost);
...
extern "C"
void sd_t_d2_fusion_(int size_a, int size_b, int size_c, int size_f, int size_d, float* t3, float* t2, float* v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	// Call An Application
	sd_t_d2_fusion(size_a, size_b1, size_b2, size_c, size_f, size_d, t3, t2, v2, cond_kernel_1, opt_register_transpose);
}
