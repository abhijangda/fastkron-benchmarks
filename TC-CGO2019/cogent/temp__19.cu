...
#define SIZE_INT_UNIT_1 SIZE_SLICE_1_D * SIZE_SLICE_1_E
...
int size_internal)
{
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['a', 'd', 'e', 'c']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['e', 'b1', 'b2', 'd']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			sm_a[threadIdx.y][threadIdx.x + ll * 16] = dev_t2[blk_idx_a * SIZE_SLICE_1_A + idx_a + (((blk_idx_c * SIZE_SLICE_1_C + ll) * size_e) * size_d) * size_a + const_internal_t2_offset[threadIdx.y + l]];
		}
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			sm_b[threadIdx.y][threadIdx.x + ll * 16] = dev_v2[(blk_idx_b1 * SIZE_SLICE_1_B1 + idx_a + (blk_idx_b2 * SIZE_SLICE_1_B2 + ll) * size_b1) * size_e + const_internal_v2_offset[threadIdx.y + l]];
		}
		__syncthreads();
		//---------------------------------------------------------------------------------------------------
...
int size_internal)
{
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			// Exception: Temp. version!: threadIdx.y + l
			// Exception: Temp. version!: idx_a < rng_a
			sm_a[threadIdx.y][threadIdx.x + ll * 16] = dev_t2[blk_idx_a * SIZE_SLICE_1_A + idx_a + (((blk_idx_c * SIZE_SLICE_1_C + ll) * size_e) * size_d) * size_a + dev_internal_offset_t2[threadIdx.y + l]];
		}
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			// Exception: Temp. version!: threadIdx.y + l
			// Exception: Temp. version!: idx_a < rng_b1
			sm_b[threadIdx.y][threadIdx.x + ll * 16] = dev_v2[(blk_idx_b1 * SIZE_SLICE_1_B1 + idx_a + (blk_idx_b2 * SIZE_SLICE_1_B2 + ll) * size_b1) * size_e + dev_internal_offset_v2[threadIdx.y + l]];
		}
		__syncthreads();
		//---------------------------------------------------------------------------------------------------
...
void sd_t_d2_fusion(int size_a, int size_b1, int size_b2, int size_c, int size_d, int size_e, double* t3, double* host_t2, double* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	cudaMalloc((void**) &dev_t2, sizeof(double) * size_c * size_e * size_d * size_a);
	cudaMalloc((void**) &dev_v2, sizeof(double) * size_d * size_b2 * size_b1 * size_e);

	// cudaMemcpy()
	cudaMemcpy(dev_t3, t3, sizeof(double) * size_a * size_b1 * size_b2 * size_c, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_c * size_e * size_d * size_a, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_d * size_b2 * size_b1 * size_e, cudaMemcpyHostToDevice);

	// Related to Kernels
	// There are 1 Basic Kernels
	long long int tmp_operations = 2 * (long long int)((long long int)(size_a * size_b1 * size_b2 * size_c) * size_d) * size_e;
	printf ("========================================= fusedKernels =============================================\n");
	...
	int size_internal = size_d * size_e;
	...
	{
		host_internal_left_offset[idx_e + (idx_d) * size_e] 	= (idx_d + (idx_e) * size_d) * size_a;
		host_internal_right_offset[idx_e + (idx_d) * size_e] 	= idx_e + (((idx_d) * size_b2) * size_b1) * size_e;
	}
	...
	if (size_a % SIZE_SLICE_1_A == 0 && size_b1 % SIZE_SLICE_1_B1 == 0 && size_b2 % SIZE_SLICE_1_B2 == 0 && size_c % SIZE_SLICE_1_C == 0)
	{
		...
		if (size_d % SIZE_SLICE_1_D == 0 && size_e % SIZE_SLICE_1_E == 0)
		{
			...
			{
				kernel__1_tex_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b1, size_b2, size_c, size_d, size_e, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b1, SIZE_SLICE_1_B1), CEIL(size_b2, SIZE_SLICE_1_B2), CEIL(size_c, SIZE_SLICE_1_C), dev_internal_offset_t2, dev_internal_offset_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
			}
			else
			{
				kernel__1_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b1, size_b2, size_c, size_d, size_e, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b1, SIZE_SLICE_1_B1), CEIL(size_b2, SIZE_SLICE_1_B2), CEIL(size_c, SIZE_SLICE_1_C), stride_reg_x_1, stride_reg_y_1, size_internal);
			}
		}
		else
		{
			...
			if (size_internal > MAX_CONST_LEN)
			{
				kernel__2_tex_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b1, size_b2, size_c, size_d, size_e, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b1, SIZE_SLICE_1_B1), CEIL(size_b2, SIZE_SLICE_1_B2), CEIL(size_c, SIZE_SLICE_1_C), dev_internal_offset_t2, dev_internal_offset_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
			}
			else
			{
				kernel__2_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b1, size_b2, size_c, size_d, size_e, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b1, SIZE_SLICE_1_B1), CEIL(size_b2, SIZE_SLICE_1_B2), CEIL(size_c, SIZE_SLICE_1_C), stride_reg_x_1, stride_reg_y_1, size_internal);
			}
		}
	}
	else
	{
		...
		if (size_d % SIZE_SLICE_1_D == 0 && size_e % SIZE_SLICE_1_E == 0)
		{
			...
			if (size_internal > MAX_CONST_LEN)
			{
				kernel__3_tex_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b1, size_b2, size_c, size_d, size_e, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b1, SIZE_SLICE_1_B1), CEIL(size_b2, SIZE_SLICE_1_B2), CEIL(size_c, SIZE_SLICE_1_C), dev_internal_offset_t2, dev_internal_offset_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
			}
			else
			{
				kernel__3_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b1, size_b2, size_c, size_d, size_e, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b1, SIZE_SLICE_1_B1), CEIL(size_b2, SIZE_SLICE_1_B2), CEIL(size_c, SIZE_SLICE_1_C), stride_reg_x_1, stride_reg_y_1, size_internal);
			}
		}
		else
		{
			...
			if (size_internal > MAX_CONST_LEN)
			{
				kernel__4_tex_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b1, size_b2, size_c, size_d, size_e, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b1, SIZE_SLICE_1_B1), CEIL(size_b2, SIZE_SLICE_1_B2), CEIL(size_c, SIZE_SLICE_1_C), dev_internal_offset_t2, dev_internal_offset_v2, stride_reg_x_1, stride_reg_y_1, size_internal);
			}
			else
			{
				kernel__4_1<<<gridsize_1, blocksize_1>>>(dev_t3, dev_t2, dev_v2, size_a, size_b1, size_b2, size_c, size_d, size_e, CEIL(size_a, SIZE_SLICE_1_A), CEIL(size_b1, SIZE_SLICE_1_B1), CEIL(size_b2, SIZE_SLICE_1_B2), CEIL(size_c, SIZE_SLICE_1_C), stride_reg_x_1, stride_reg_y_1, size_internal);
			}
		}
	}
