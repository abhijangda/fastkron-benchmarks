...
int size_internal)
{
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'y', 't2', ['g', 'f', 'b', 'c']], [16, 'STR_SD2_V2_H7', 'x', 'v2', ['d', 'e', 'g', 'a']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			sm_a[threadIdx.x][threadIdx.y + ll * 16] = dev_t2[(blk_idx_f * SIZE_SLICE_1_F + idx_f + (blk_idx_b * SIZE_SLICE_1_B + ll + (blk_idx_c * SIZE_SLICE_1_C + 0) * size_b) * size_f) * size_g + (threadIdx.x + l)];
		}
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			sm_b[threadIdx.y][threadIdx.x + ll * 16] = dev_v2[blk_idx_d * SIZE_SLICE_1_D + ll + (blk_idx_e * SIZE_SLICE_1_E + 0 + ((blk_idx_a * SIZE_SLICE_1_A + idx_a) * size_g) * size_e) * size_d + (threadIdx.y + l) * stride_int_v2];
		}
		__syncthreads();
		//---------------------------------------------------------------------------------------------------
		...
		{
			temp_bv[0] = sm_a[ll][idx_f + (idx_c) * SIZE_SLICE_1_F + 0];
			temp_bv[1] = sm_a[ll][idx_f + (idx_c) * SIZE_SLICE_1_F + 16];
			temp_bv[2] = sm_a[ll][idx_f + (idx_c) * SIZE_SLICE_1_F + 32];
			temp_bv[3] = sm_a[ll][idx_f + (idx_c) * SIZE_SLICE_1_F + 48];
			temp_bv[4] = sm_a[ll][idx_f + (idx_c) * SIZE_SLICE_1_F + 64];
			temp_bv[5] = sm_a[ll][idx_f + (idx_c) * SIZE_SLICE_1_F + 80];

			for (int xx = 0; xx < 6; xx++) // (1)
			{
				temp_av = sm_b[ll][idx_e + (idx_a) * SIZE_SLICE_1_E + (xx * 16)];
...
void sd_t_d2_fusion(int size_a, int size_b, int size_c, int size_d, int size_e, int size_f, int size_g, double* t3, double* host_t2, double* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	cudaMalloc((void**) &dev_t2, sizeof(double) * size_c * size_b * size_f * size_g);
	cudaMalloc((void**) &dev_v2, sizeof(double) * size_a * size_g * size_e * size_d);

	// cudaMemcpy()
	cudaMemcpy(dev_t3, t3, sizeof(double) * size_a * size_b * size_c * size_d * size_e * size_f, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_c * size_b * size_f * size_g, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_a * size_g * size_e * size_d, cudaMemcpyHostToDevice);
