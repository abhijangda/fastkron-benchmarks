...
#define SIZE_TB_1_X 	SIZE_SLICE_1_A * SIZE_SLICE_1_E
#define SIZE_TB_1_Y 	SIZE_SLICE_1_F * SIZE_SLICE_1_C
...
int size_internal)
{
	...
	int idx_e = threadIdx.x / SIZE_SLICE_1_A;
	int idx_f = threadIdx.y % SIZE_SLICE_1_F;
	int idx_c = threadIdx.y / SIZE_SLICE_1_F;
	...
	int t3_base_thread = blk_idx_a * SIZE_SLICE_1_A + idx_a + (blk_idx_b * SIZE_SLICE_1_B + (blk_idx_c * SIZE_SLICE_1_C + idx_c + (blk_idx_d * SIZE_SLICE_1_D + (blk_idx_e * SIZE_SLICE_1_E + idx_e + (blk_idx_f * SIZE_SLICE_1_F + idx_f) * size_e) * size_d) * size_c) * size_b) * size_a;
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['d', 'e', 'g', 'a']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['g', 'f', 'b', 'c']], '-=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			sm_a[threadIdx.y][threadIdx.x + ll * 16] = dev_t2[blk_idx_d * SIZE_SLICE_1_D + ll + (blk_idx_e * SIZE_SLICE_1_E + 0 + ((blk_idx_a * SIZE_SLICE_1_A + idx_a) * size_g) * size_e) * size_d + (threadIdx.y + l) * stride_int_t2];
		}
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			sm_b[threadIdx.x][threadIdx.y + ll * 16] = dev_v2[(blk_idx_f * SIZE_SLICE_1_F + idx_f + (blk_idx_b * SIZE_SLICE_1_B + ll + (blk_idx_c * SIZE_SLICE_1_C + 0) * size_b) * size_f) * size_g + (threadIdx.x + l)];
		}
		__syncthreads();
		//---------------------------------------------------------------------------------------------------
		...
		{
			temp_bv[0] = sm_b[ll][idx_f + (idx_c) * SIZE_SLICE_1_F + 0];
			temp_bv[1] = sm_b[ll][idx_f + (idx_c) * SIZE_SLICE_1_F + 16];
			temp_bv[2] = sm_b[ll][idx_f + (idx_c) * SIZE_SLICE_1_F + 32];
			temp_bv[3] = sm_b[ll][idx_f + (idx_c) * SIZE_SLICE_1_F + 48];
			temp_bv[4] = sm_b[ll][idx_f + (idx_c) * SIZE_SLICE_1_F + 64];
			temp_bv[5] = sm_b[ll][idx_f + (idx_c) * SIZE_SLICE_1_F + 80];

			for (int xx = 0; xx < 6; xx++) // (1)
			{
				temp_av = sm_a[ll][idx_e + (idx_a) * SIZE_SLICE_1_E + (xx * 16)];
...
int size_internal)
{
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		// This Part is for Loading Input-Left
		// tc_gen_code_Kernel_Load_Inputs_Abstracts()
		if (0 < rng_e && idx_a < rng_a && threadIdx.y < SIZE_INT_UNIT_1 - internal_upperbound)
		for (int ll = 0; ll < rng_d; ll++)
		{
			// ['d', 'e', 'g', 'a']
			// Exception: Temp. version!: threadIdx.y + l
...
void sd_t_d2_fusion(int size_a, int size_b, int size_c, int size_d, int size_e, int size_f, int size_g, double* t3, double* host_t2, double* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	cudaMalloc((void**) &dev_t2, sizeof(double) * size_a * size_g * size_e * size_d);
	cudaMalloc((void**) &dev_v2, sizeof(double) * size_c * size_b * size_f * size_g);

	// cudaMemcpy()
	cudaMemcpy(dev_t3, t3, sizeof(double) * size_a * size_b * size_c * size_d * size_e * size_f, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_a * size_g * size_e * size_d, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_c * size_b * size_f * size_g, cudaMemcpyHostToDevice);
