...
#define SIZE_REG_1_Y 	SIZE_SLICE_1_D
...
int size_internal)
{
	...
	int t3_base_thread = blk_idx_a * SIZE_SLICE_1_A + idx_a + (blk_idx_b * SIZE_SLICE_1_B + idx_b + (blk_idx_c * SIZE_SLICE_1_C + (blk_idx_d * SIZE_SLICE_1_D) * size_c) * size_b) * size_a;
	...
	// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['a', 'e', 'f', 'c']], [16, 'STR_SD2_V2_H7', 'y', 'v2', ['f', 'b', 'e', 'd']], '+=']
	#pragma unroll 1
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			sm_a[threadIdx.y][threadIdx.x + ll * 16] = dev_t2[blk_idx_a * SIZE_SLICE_1_A + idx_a + (((blk_idx_c * SIZE_SLICE_1_C + ll) * size_f) * size_e) * size_a + const_internal_t2_offset[threadIdx.y + l]];
		}
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			sm_b[threadIdx.y][threadIdx.x + ll * 16] = dev_v2[(blk_idx_b * SIZE_SLICE_1_B + idx_a + ((blk_idx_d * SIZE_SLICE_1_D + ll) * size_e) * size_b) * size_f + const_internal_v2_offset[threadIdx.y + l]];
		}
		__syncthreads();
		//---------------------------------------------------------------------------------------------------
...
int size_internal)
{
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		// This Part is for Loading Input-Right
		// tc_gen_code_Kernel_Load_Inputs_Abstracts()
		if (idx_a < rng_b && threadIdx.y < SIZE_INT_UNIT_1 - internal_upperbound)
		for (int ll = 0; ll < rng_d; ll++)
		{
			// ['f', 'b', 'e', 'd']
			// Exception: Temp. version!: threadIdx.y + l
...
int size_internal)
{
	...
	for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1)
	{
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			// Exception: Temp. version!: threadIdx.y + l
			// Exception: Temp. version!: idx_a < rng_a
			sm_a[threadIdx.y][threadIdx.x + ll * 16] = dev_t2[blk_idx_a * SIZE_SLICE_1_A + idx_a + (((blk_idx_c * SIZE_SLICE_1_C + ll) * size_f) * size_e) * size_a + dev_internal_offset_t2[threadIdx.y + l]];
		}
		...
		for (int ll = 0; ll < 6; ll++)
		{
			...
			// Exception: Temp. version!: threadIdx.y + l
			// Exception: Temp. version!: idx_a < rng_b
			sm_b[threadIdx.y][threadIdx.x + ll * 16] = dev_v2[(blk_idx_b * SIZE_SLICE_1_B + idx_a + ((blk_idx_d * SIZE_SLICE_1_D + ll) * size_e) * size_b) * size_f + dev_internal_offset_v2[threadIdx.y + l]];
		}
		__syncthreads();
		//---------------------------------------------------------------------------------------------------
...
extern "C"
void sd_t_d2_fusion(int size_a, int size_b, int size_c, int size_d, int size_e, int size_f, double* t3, double* host_t2, double* host_v2, int cond_kernel_1, int opt_register_transpose)
{
	...
	// cudaMalloc()
	cudaMalloc((void**) &dev_t3, sizeof(double) * size_a * size_b * size_c * size_d);
	cudaMalloc((void**) &dev_t2, sizeof(double) * size_c * size_f * size_e * size_a);
	cudaMalloc((void**) &dev_v2, sizeof(double) * size_d * size_e * size_b * size_f);

	// cudaMemcpy()
	cudaMemcpy(dev_t3, t3, sizeof(double) * size_a * size_b * size_c * size_d, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_c * size_f * size_e * size_a, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_d * size_e * size_b * size_f, cudaMemcpyHostToDevice);

	// Related to Kernels
	// There are 1 Basic Kernels
	long long int tmp_operations = 2 * (long long int)((long long int)(size_a * size_b * size_c * size_d) * size_e) * size_f;
	printf ("========================================= fusedKernels =============================================\n");
	...
	{
		host_internal_left_offset[idx_f + (idx_e) * size_f] 	= (idx_e + (idx_f) * size_e) * size_a;
		host_internal_right_offset[idx_f + (idx_e) * size_f] 	= idx_f + ((idx_e) * size_b) * size_f;
	}
	...
	cudaMemcpy(t3, dev_t3, sizeof(double) * (size_a * size_b * size_c * size_d), cudaMemcpyDeviceToHost);
